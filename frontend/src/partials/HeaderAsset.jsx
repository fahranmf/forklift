...
function Header({ variant = "default", title, path}) {
  const navigate = useNavigate();

  const handleBack = () => {
    if (path) {
      navigate(path); // kalau ada path, ke sana
    } else {
      navigate(-1); // kalau enggak ada path, fallback ke history back
    }
  };
          ...
          <div className="flex">
            <div className="flex items-center gap-3">
              <>
                <button
                  onClick={handleBack}
                  className="inline-flex items-center p-2 rounded-lg hover:bg-gray-200 text-gray-800 dark:text-gray-100 dark:hover:bg-gray-700 transition"
                >
                  <svg
                ...
                <h1 className="text-lg text-gray-800 dark:text-gray-100 font-semibold">{title}</h1>
              </>
            </div>
          </div>
